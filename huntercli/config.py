...
class Settings:
    ...
    #: Считать зарплаты по вакансиям, подобранным под резюме. Стоит три
    #: запроса на резюме в сутки; кому не нужно — выключает.
    salary_enabled: bool = True
    #: Убирать мёртвые обращения из активного списка. Скрытие необратимо,
    #: поэтому выключатель обязателен: передумать можно только заранее.
    cleanup_enabled: bool = True
    #: Сколько суток без движения считать «ответа не будет».
