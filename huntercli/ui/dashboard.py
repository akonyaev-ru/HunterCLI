...
from .. import APP_NAME, history, salary
...
class Dashboard:
    ...
    def _stats_rows(report: "history.Report | None", room: int) -> list[tuple[str, str]]:
        ...
        # Свой заголовок обязателен: числа тут за всё время, а сверху блок за
        # неделю, и без подписи «355» читается как недельное. Слова берём с
        # сайта — «отклики», «приглашения», а не выдуманные «обращения».
        footer = [("", ""), ("За всё время", ""),
                  ("Просмотров", str(report.total_views))]
        if report.has_talks:
            # «39 из 355» без слова читалось как недельное число просмотров.
            # Слово берём с сайта — «отклики», а не выдуманные «обращения».
            footer.append(("Приглашений",
                           f"{report.invitations} из {report.talks} откликов"))
        # Наши записи начинаются позже счётчиков сайта: недельные приросты
        # считаются от этой даты, а числа выше берутся у сервиса целиком.
        footer.append(("Наблюдаем с", _day(report.since)))
        # Зарплаты — числа рыночные, а всё вокруг собственные. Свой заголовок
        # обязателен: без него «185 000» читается как что-то про этот аккаунт.
        # Тот же урок, что с «39 из 355» (журнал 2026-08-28).
        # Доля указавших идёт вплотную к цифрам и не выбрасывается отдельно:
        # медиана по трети вакансий без этой строки — обман, а не сводка.
        # Доля указавших стоит в одной строке с размахом, а не отдельной: на
        # своей строке она уходила первой при нехватке места, а медиана по
        # трети вакансий без неё — обман, а не сводка. Четыре строки вместо
        # пяти — разница между «видно на обычном окне» и «не видно никогда».
        money: list[tuple[str, str]] = []
        if report.has_salary:
            money = [
                ("", ""),
                ("Зарплаты по вашему профилю", ""),
                ("Середина", salary.human(report.salary_median)),
                ("Половина вакансий",
                 f"{salary.human(report.salary_low)} – {salary.human(report.salary_high)}"
                 f"  ·  указана у {report.salary_count} из {report.salary_total}"),
            ]

        if room < len(rows) + len(footer):
            footer = []
        # Зарплаты идут раньше разбивки по резюме и место резервируют раньше:
        # это сводка, как и итоги, а разбивка — список, и она для того и
        # обрезается (решение 26). Раздел выводится целиком или никак:
        # заголовок с одной строкой под ним хуже отсутствующего.
        #
        # `not footer` в условии обязателен. Без него выходило, что на узком
        # окне зарплаты занимали место выброшенных итогов: на 11 строках они
        # были, на 14 пропадали, а на 16 появлялись снова. Больше места —
        # меньше содержимого, и объяснить это владельцу было бы нечем.
        # Свои итоги старше рыночных цифр: нет их — нет и зарплат.
        if not footer or room < len(rows) + len(footer) + len(money):
            money = []

        # Разбивке нужны отбивка, заголовок и хотя бы одно резюме. Заголовок
        # без единой строки под ним не выводим никогда — то же правило, что у
        # панелей на главном экране.
        free = room - len(rows) - len(footer) - len(money) - 2
