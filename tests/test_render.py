...
def run() -> bool:
    ...
    report.section("Зарплаты на экране статистики")
    rich_money = _report(salary_median=185000, salary_low=150000,
                         salary_high=230000, salary_count=96, salary_total=300)
    money = _stats(rich_money)
    report.check("середина показана", "185 000" in money, "-> нет «185 000»")
    report.check("половина вакансий показана размахом",
                 "150 000 – 230 000" in money, "-> нет вилки")
    # Медиана без доли указавших — обман: зарплату публикует меньшинство.
    report.check("доля указавших рядом с цифрами", "96 из 300" in money,
                 "-> нет «96 из 300»")
    # Числа рыночные, а рядом собственные: без своего заголовка «185 000»
    # читается как что-то про этот аккаунт.
    report.check("у раздела свой заголовок",
                 "Зарплаты по вашему профилю" in money)
    report.check("заголовок раньше чисел",
                 money.find("Зарплаты по вашему") < money.find("185 000"))
    # Не считали — раздела нет вовсе; ноль и «не считали» это разные вещи.
    quiet = _stats(_report())
    report.check("без подсчёта раздела нет", "Зарплаты по вашему" not in quiet)
    report.check("остальная статистика на месте", "+85" in quiet)
    empty_money = _stats(_report(salary_count=0, salary_total=300))
    report.check("зарплат не нашлось — раздела тоже нет",
                 "Зарплаты по вашему" not in empty_money)

    # Раздел либо целиком, либо никак: заголовок с одной строкой хуже, чем
    # его отсутствие. На низком окне он обязан уйти весь.
    for height in (40, 32, 26, 22, 18, 14):
        text = _stats(rich_money, 120, height)
        lines = [ln.rstrip() for ln in text.splitlines()]
        head = "Зарплаты по вашему профилю" in text
        report.check(f"120x{height}: раздел зарплат целиком или никак",
                     not head or ("указана у" in text and "Середина" in text),
                     f"-> заголовок={head}")
        # Свои итоги старше рыночных: раздела зарплат без них быть не может,
        # иначе на узком окне они занимали бы место выброшенных итогов.
        report.check(f"120x{height}: зарплаты не вытесняют собственные итоги",
