...
* 🪟 **Состояние в заголовке окна.** Свёрнутая программа сообщает о себе в панели задач: сколько осталось до поднятия, пауза или обрыв связи — разворачивать окно для этого не нужно.
* 🔍 **Автоматический поиск резюме.** Список загружается из профиля; любое резюме отключается одной клавишей.
* 📈 **Статистика просмотров и приглашений.** Программа ежедневно запоминает счётчики каждого резюме и по клавише `S` показывает, что дала неделя: прирост просмотров, сравнение с предыдущей неделей, сколько просмотров принесло одно поднятие, сколько пришло приглашений и какое резюме работает лучше остальных. Приглашения показываются числом — «39 из 355 обращений»; кто именно позвал и на какую вакансию, смотрите на сайте.
* 👥 **Несколько аккаунтов одновременно.** Каждый работает на собственной вкладке в том же окне: отдельный доступ, свой перечень резюме и своя статистика.
...
| `N` | Подключить дополнительный аккаунт |
| `Tab` / `Shift+Tab` | Перейти к следующей или предыдущей вкладке |
| `D` | Отключить аккаунт открытой вкладки (с подтверждением) |
| `1`…`9` | Включить или отключить резюме с указанным номером |
| `S` | Статистика просмотров и приглашений за неделю |
| `L` | Открыть файл журнала |
| `H` | Справка |

## 👥 Несколько аккаунтов

Клавиша `N` подключает дополнительный аккаунт: открывается то же окно входа, а после успешной авторизации аккаунт появляется на собственной вкладке в верхней части экрана. Переключение между вкладками выполняется клавишей `Tab`, отключение аккаунта — клавишей `D` с подтверждением.

Аккаунты работают одновременно и независимо друг от друга: у каждого собственный токен, собственный перечень выбранных резюме и собственная статистика. Общими остаются настройки программы и журнал событий — записи в нём подписываются именем владельца аккаунта.

Одновременно допускается до восьми аккаунтов. Повторное подключение уже подключённого аккаунта отклоняется: два автопилота на одних и тех же резюме друг другу мешают.
...
|---|---|
| `config.json` | Зашифрованные токены подключённых аккаунтов, перечень выбранных резюме, статистика |
| `stats.json` | История просмотров и обращений к работодателям: по одному срезу счётчиков в сутки, хранится год. Открытые числа, без шифрования. Переписка и названия вакансий сюда не попадают |
| `hunter.log` | Журнал событий; при достижении 1 МБ переносится в `hunter.log.1` |
| `%LOCALAPPDATA%\HunterCLI\` | Сессия окна авторизации, благодаря которой повторный вход выполняется в один шаг. У каждого аккаунта она своя |
...
| `managed_resumes` | `null` | Задаётся отдельно для каждого аккаунта в разделе `accounts`. `null` соответствует всем резюме; допускается список идентификаторов |
