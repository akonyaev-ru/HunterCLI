...
2. Скачайте `HunterCLI.exe` — распаковывать ничего не нужно — и положите в любую папку, где у вас есть права на запись (рядом появятся `config.json` и `hunter.log`).
...
После прогона `tests\render-preview.txt` показывает, как выглядит интерфейс на разной ширине окна.

Картинку для этого README пересобирает `python tools\make_preview.py` — она рисуется настоящим кодом дашборда, поэтому не устаревает вслед за интерфейсом. Для перевода в PNG используется установленный в Windows Microsoft Edge.
